...
export default {
  ...
  methods: {
    async save (form) {
      ...
      await axios.post(adonai.url + 'tickets', form, { headers: { Authorization: 'Bearer ' + this.user.token } }).then(res => {
    ...
    getbyId (id) {
      this.openloading = true
      axios.get(adonai.url + 'tickets/' + id, { headers: { Authorization: 'Bearer ' + this.user.token } }).then(res => {
